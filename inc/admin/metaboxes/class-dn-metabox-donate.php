...
if ( ! class_exists( 'DN_MetaBox_Donate' ) ) {
	...
	class DN_MetaBox_Donate extends DN_MetaBox_Base {
		...
		public function update_donate( $post_id ) {
			...
			/* donor */
			update_post_meta( $post_id, 'thimpress_donate_donor_id', isset( $_POST['thimpress_donate_donor_id'] ) ? esc_attr( absint( sanitize_params_submitted($_POST['thimpress_donate_donor_id']) ) ) : 0 );

			$total = 0;
			if ( $donate_type === 'system' || ( ! isset( $_POST['donate_item'] ) || empty( $_POST['donate_item'] ) ) ) {
				$donate->remove_donate_items();
				$total = isset( $_POST['thimpress_donate_total'] ) ? esc_attr( floatval( sanitize_params_submitted($_POST['thimpress_donate_total']) ) ) : 0;
			} else if ( $donate_type === 'campaign' ) {
                            $_POST['donate_item'] = sanitize_params_submitted($_POST['donate_item']); 
