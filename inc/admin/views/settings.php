...
<?php if( $donate_setting ): ?>
	...
	<form method="POST" name="tp_donate_options">
		...
		<div class="wrap tp_donate_setting_wrapper">
			...
			<h2 class="nav-tab-wrapper">
				<?php foreach( $donate_setting as $key => $title ): ?>
					...
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=tp_donate_setting&tab=' . $key ) ); ?>" class="nav-tab<?php echo $current_tab === $key ? ' nav-tab-active' : '' ?>">
						<?php printf( '%s', $title ) ?>
					</a>
			...
			<div class="tp_donate_wrapper_content">
				...
				<?php foreach( $donate_setting as $key => $title ): ?>
					...
					<div id="<?php echo esc_attr( $key ) ?>">
						<?php echo do_action( 'donate_admin_setting_' . $key . '_content' ); ?>
					</div>
